    ...
    case 'photos':
      ...
      if ($_POST["multiple"] == "true") {
        ...
        foreach ($files as $file) {
          ...
          if ($_POST['blur'] == "true") {
            if (!in_array($image->_img_type, ["image/gif", "image/webp"])) {
              watermark_image($file["tmp_name"], $image->_img_type);
              blur_image($file["tmp_name"], $image->_img_type);
          ...
          if ($system['s3_enabled']) {
            ...
            /* DigitalOcean */
            digitalocean_space_upload($file["tmp_name"], $file_name, mime_content_type($file['tmp_name']));
          } elseif ($system['wasabi_enabled']) {
            /* Wasabi */
            ...
            /* Yandex Cloud */
            yandex_cloud_upload($file["tmp_name"], $file_name, mime_content_type($file['tmp_name']));
          } elseif ($system['ftp_enabled']) {
            /* FTP */
            ...
            if (in_array($image->_img_type, ["image/gif", "image/webp"])) {
              ...
              if ($image_watermarked || $image_source_blured) {
                $image = new Image($file["tmp_name"]);
              }
          ...
          $db->query(sprintf("INSERT INTO users_uploads (user_id, file_name, file_size, insert_date) VALUES (%s, %s, %s, %s)", secure($user->_data['user_id']), secure($file_name), secure($file["size"]), secure($date)));
        ...
        if ($_POST['blur'] == "true") {
          if (!in_array($image->_img_type, ["image/gif", "image/webp"])) {
            /* watermark & blur */
            watermark_image($_FILES['file']['tmp_name'], $image->_img_type);
            blur_image($_FILES['file']['tmp_name'], $image->_img_type);
            $image_source_blured = 1;
          }
        }
        ...
        if ($system['s3_enabled']) {
          ...
          /* DigitalOcean */
          digitalocean_space_upload($_FILES['file']['tmp_name'], $file_name, mime_content_type($_FILES['file']['tmp_name']));
        } elseif ($system['wasabi_enabled']) {
          /* Wasabi */
          ...
          /* Yandex Cloud */
          yandex_cloud_upload($_FILES['file']['tmp_name'], $file_name, mime_content_type($_FILES['file']['tmp_name']));
        } elseif ($system['ftp_enabled']) {
          /* FTP */
        ...
        // log the file size inside the database (users_uploads)
        $db->query(sprintf("INSERT INTO users_uploads (user_id, file_name, file_size, insert_date) VALUES (%s, %s, %s, %s)", secure($user->_data['user_id']), secure($file_name), secure($_FILES['file']['size']), secure($date)));
        ...
        switch ($_POST['handle']) {
          case 'cover-user':
            ...
            $db->query(sprintf("INSERT INTO posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($user->_data['user_album_covers'], 'int'), secure($file_name), secure($image_blured)));
            $photo_id = $db->insert_id;
            /* update user cover */
            $db->query(sprintf("UPDATE users SET user_cover = %s, user_cover_id = %s WHERE user_id = %s", secure($file_name), secure($photo_id, 'int'), secure($user->_data['user_id'], 'int')));
            break;
          ...
          case 'picture-user':
            ...
            $db->query(sprintf("INSERT INTO posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($user->_data['user_album_pictures'], 'int'), secure($file_name), secure($image_blured)));
            $photo_id = $db->insert_id;
            /* delete old cropped picture from uploads folder */
            delete_uploads_file($user->_data['user_picture_raw']);
            /* update user profile picture */
            $db->query(sprintf("UPDATE users SET user_picture = %s, user_picture_id = %s WHERE user_id = %s", secure($file_name), secure($photo_id, 'int'), secure($user->_data['user_id'], 'int')));
            break;
          ...
          case 'cover-page':
            ...
            $post_id = $db->insert_id;
            /* insert new cover photo to album */
            $db->query(sprintf("INSERT INTO posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($page['page_album_covers'], 'int'), secure($file_name), secure($image_blured)));
            $photo_id = $db->insert_id;
            /* update page cover */
            $db->query(sprintf("UPDATE pages SET page_cover = %s, page_cover_id = %s WHERE page_id = %s", secure($file_name), secure($photo_id, 'int'), secure($page['page_id'], 'int')));
            break;
          ...
          case 'picture-page':
            ...
            $db->query(sprintf("INSERT INTO posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($page['page_album_pictures'], 'int'), secure($file_name), secure($image_blured)));
            $photo_id = $db->insert_id;
            /* delete old cropped picture from uploads folder */
            delete_uploads_file($page['page_picture']);
            /* update page picture */
            $db->query(sprintf("UPDATE pages SET page_picture = %s, page_picture_id = %s WHERE page_id = %s", secure($file_name), secure($photo_id, 'int'), secure($page['page_id'], 'int')));
            break;
          ...
          case 'cover-group':
            ...
            $db->query(sprintf("INSERT INTO posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($group['group_album_covers'], 'int'), secure($file_name), secure($image_blured)));
            $photo_id = $db->insert_id;
            /* update group cover */
            $db->query(sprintf("UPDATE `groups` SET group_cover = %s, group_cover_id = %s WHERE group_id = %s", secure($file_name), secure($photo_id, 'int'), secure($group['group_id'], 'int')));
            break;
          ...
          case 'picture-group':
            ...
            $db->query(sprintf("INSERT INTO posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($group['group_album_pictures'], 'int'), secure($file_name), secure($image_blured)));
            $photo_id = $db->insert_id;
            /* delete old cropped picture from uploads folder */
            delete_uploads_file($group['group_picture']);
            /* update group picture */
            $db->query(sprintf("UPDATE `groups` SET group_picture = %s, group_picture_id = %s WHERE group_id = %s", secure($file_name), secure($photo_id, 'int'), secure($group['group_id'], 'int')));
            break;
          ...
          case 'cover-event':
            ...
            $db->query(sprintf("INSERT INTO posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($event['event_album_covers'], 'int'), secure($file_name), secure($image_blured)));
            $photo_id = $db->insert_id;
            /* update event cover */
            $db->query(sprintf("UPDATE `events` SET event_cover = %s, event_cover_id = %s WHERE event_id = %s", secure($file_name), secure($photo_id, 'int'), secure($event['event_id'], 'int')));
            break;
        }
 ...
 * @return float
 */
function get_payment_vat_value($amount)
{
  global $system;
  $vat = 0;
 ...
 * @return float
 */
function get_payment_fees_value($amount)
{
  global $system;
  $fees = 0;
 ...
 * @return void
 */
function user_access($is_ajax = false, $bypass_subscription = false, $oauth_app_id = null)
{
  global $user, $system;
  if ($is_ajax) {
...
function get_user_language_country()
{
  if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
    ...
    $userLanguages = explode(',', $languages);
    $primaryLanguage = explode(';', $userLanguages[0])[0];
    $primaryLanguage = strtolower($primaryLanguage);
    $primaryLanguage = str_replace('-', '_', $primaryLanguage);
    /* check if contains ar_xx then replace it ar_sa */
    if (strpos($primaryLanguage, 'ar_') !== false) {
      $primaryLanguage = 'ar_sa';
    }
    /* extract language and country code if available */
    $languageParts = explode('_', $primaryLanguage);
    $languageCode = $languageParts[0];
    $countryCode = isset($languageParts[1]) ? strtoupper($languageParts[1]) : null;
    return [
