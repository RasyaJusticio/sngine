...
try {
  ...
  switch ($_GET['do']) {
    case 'edit':
      ...
      /* update */
      $db->query(sprintf("UPDATE system_time_zones SET system_countries.time_zone_name = %s, time_zone_order = %s, utc_offset = %s `enabled` = %s WHERE time_zone_id = %s", secure($_POST['default']), secure($_POST['enabled']), secure($_POST['country_code']), secure($_POST['country_name']), secure($_POST['phone_code']), secure($_POST['country_vat'], 'int'), secure($_POST['country_order'], 'int'), secure($_GET['id'], 'int')));
      /* return */
